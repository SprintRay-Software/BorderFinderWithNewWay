    ...
    if(isRead)
        {
            ...
            qDebug()<<"read_mesh";
            QString Borderfinder_path=QCoreApplication::applicationDirPath()+"/Borderfinder";
            QDir Borderfinder_dir(Borderfinder_path);
            std::cout<<"wangjx dir=:"<<Borderfinder_path.toStdString()<<std::endl;
            try {
                if(!Borderfinder_dir.exists())
                {
                    std::cout<<"wangjx dir not exist"<<std::endl;
                    if(Borderfinder_dir.mkdir(Borderfinder_path))
                    {
                        std::cout<<"Create directory"<<Borderfinder_path.toStdString()<<"Succeed"<<std::endl;
                    }
                    else
                    {
                        std::cout<<"Create directory"<<Borderfinder_path.toStdString()<<"Failed and Progress Quit..."<<std::endl;
                        exit(-1);
                    }
                }
            }  catch (...) {
