...
vector<Node> GrahamScan::readFile(string filename)
{
    ifstream myFile;
    string temp;
    ...
    string str = code->fromUnicode(QString::fromStdString(filename)).data();
    ...
    while (numNodes--) {
        ...
        iss >> subStr;
        float xVal, yVal;
        sscanf(subStr.c_str(), "[%f,%f]", &xVal, &yVal);
        graph.push_back(Node(xVal, yVal));;
        cout<<"Node :: "<<"( "<< xVal <<", " << yVal <<")";
    }
...
Node GrahamScan::outputFile(string filename)
{
    QTextCodec *code = QTextCodec::codecForName("GBK");
    string str = code->fromUnicode(QString::fromStdString(filename)).data();
